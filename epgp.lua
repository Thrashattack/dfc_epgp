...
local function SleepCall ()
end

local function Sleep (t)
  self:ScheduleTimer(SleepCall,t)
end
...
local function AddEPGP(name, ep, gp, skip_notes)
  ...
  if(not skip_notes) then
    GS:SetNote(name, EncodeNote(total_ep + ep,
                                total_gp + gp ))
  end
  local final_ep = total_ep + ep
  local final_gp = total_gp + gp
  return ep, gp, final_ep, final_gp
end
...
function EPGP:ImportRoster(t, new_base_gp)
  ...
  local min_note = EncodeNote(global_config.min_gp, global_config.base_gp)
  for name,_ in pairs(ep_data) do
    local note = notes[name] or min_note
...
function EPGP:StandingsHideMinEPGP(val)
  if val == nil then
    return db.profile.show_min_epgp_players
  end

  db.profile.show_min_epgp_players = not not val
  DestroyStandings()
end
...
function EPGP:ResetEPGP()
  ...
  local min_note = EncodeNote(global_config.min_ep, global_config.base_gp)
  for name,_ in pairs(ep_data) do
    GS:SetNote(name, min_note)
    local ep, gp, main = self:GetEPGP(name)
    assert(main == nil, "Corrupt alt data!")
    local extra_ep = ep - global_config.min_ep
    local extra_gp = gp - global_config.base_gp
    if ep > global_config.min_ep then
      callbacks:Fire("EPAward", name, "Reset", -extra_ep, true)
    end
    if gp > global_config.base_gp then
      callbacks:Fire("GPAward", name, "Reset", -extra_gp, true)
    end
  end
...
-- Fixing this buggy function to prevent disconnect due to massive spam when decaying plus do not decay below the minimum (LOL)
-- Kudos to @4bit for the minimum decay and @Thrashattack for the delay frame
function EPGP:DecayEPGP()
  ...
  -- Queue to hold the callback data
  local awardQueue = {}

  -- Populate the queue with decay data
  for name, _ in pairs(ep_data) do
    local ep, gp, main = self:GetEPGP(name)
    assert(main == nil, "Corrupt alt data!")
  ...
  -- Frame to handle delayed dispatch
  local delayFrame = CreateFrame("Frame")
  local interval = 1.0 -- seconds between each callback
  local elapsed = 0
  local index = 1

  delayFrame:SetScript("OnUpdate", function(self, delta)
    elapsed = elapsed + delta
    if elapsed >= interval and awardQueue[index] then
      local award = awardQueue[index]
      if award.type == "EP" then
        callbacks:Fire("EPAward", award.name, award.reason, award.amount, true)
      elseif award.type == "GP" then
...
function EPGP:GetEPGP(name)
  ...
  if ep_data[name] then
    return ep_data[name], gp_data[name], main
  end
end
